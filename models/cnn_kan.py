...
class CNN_KAN(nn.Module):
    ...
    def __init__(self, output_dimesion, vocab_size, dropout_rate, emb_dim, max_len, n_filters, hidden_dim, if_cuda, init_W=None):
        # n_filter为卷积核个数
        ...
        '''Dropout Layer'''
        self.layer_kan = KAN([n_filters*3, hidden_dim, vanila_dimension], base_activation=nn.Identity)
        self.dropout = nn.Dropout(dropout_rate)

        '''Projection Layer & Output Layer'''
        # self.output_layer = nn.Linear(vanila_dimension, projection_dimension)
        self.output_layer_kan = KAN([vanila_dimension, hidden_dim, projection_dimension], base_activation=nn.Identity)
    ...
    def forward(self, inputs):
        ...
        out = F.tanh(self.layer_kan(flatten))
        out = self.dropout(out)
        out = F.tanh(self.output_layer_kan(out))
